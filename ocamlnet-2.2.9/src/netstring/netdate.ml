(* $Id: netdate.mlp 971 2006-08-24 19:29:22Z gerd $
 * ----------------------------------------------------------------------
 *
 *)
...
let tokens_list =
  [ ("january", (Month 1)); ("jan", (Month 1)); ("february", (Month 2));
    ...
    ("bst", (Zone 0100)); (*DST*) ("cet", (Zone 0100));
...
let tokens_of_string str =
  let rec scan_any (__strm : _ Stream.t) =
    ...
    | Some (('0' .. '9' as c)) ->
        (Stream.junk __strm; scan_number ((int_of_char c) - 48) __strm)
    | Some (('a' .. 'z' | 'A' .. 'Z' as c)) ->
        (Stream.junk __strm;
         let rest = __strm in
    ...
    | _ -> Stream.sempty
  and scan_number a (__strm : _ Stream.t) =
    match Stream.peek __strm with
    | Some (('0' .. '9' as c)) ->
        (Stream.junk __strm;
         scan_number ((a * 10) + ((int_of_char c) - 48)) __strm)
    | _ ->
        let rest = __strm
        in Stream.icons (Number a) (Stream.slazy (fun _ -> scan_any rest))
...
let parse str =
  ...
  let rec scan_gen (__strm : _ Stream.t) =
    match Stream.peek __strm with
    | Some (Number n) -> (Stream.junk __strm; scan_number n __strm)
    | Some (Zone tz) ->
        (Stream.junk __strm;
         let dst =
           (try scan_dst __strm
    ...
    | Some (Month mo) ->
        (Stream.junk __strm;
         (match Stream.peek __strm with
          | Some (Number md) ->
              (Stream.junk __strm; scan_date_m mo md __strm)
          | _ -> raise (Stream.Error "")))
    | Some _ -> (Stream.junk __strm; invalid_arg "Parse.date")
    | _ -> ()
  and scan_number n (__strm : _ Stream.t) =
    match Stream.peek __strm with
    | Some (Meridian mdn) ->
        (Stream.junk __strm;
    ...
    | Some Colon ->
        (Stream.junk __strm;
         (match Stream.peek __strm with
          | Some (Number m) -> (Stream.junk __strm; scan_hour n m __strm)
          | _ -> raise (Stream.Error "")))
    | Some Slash ->
        (Stream.junk __strm;
         (match Stream.peek __strm with
          | Some (Number m) -> (Stream.junk __strm; scan_date_s n m __strm)
          | _ -> raise (Stream.Error "")))
    | Some Minus -> (Stream.junk __strm; scan_date_d n __strm)
    | Some (Month mo) ->
        (Stream.junk __strm;
         let rest = __strm in (add_data ~md: n ~mo (); scan_gen rest))
    | _ -> let rest = __strm in (add_data ~y: n (); scan_gen rest)
  and scan_hour h m (__strm : _ Stream.t) =
    ...
    | Some Colon ->
        (Stream.junk __strm;
         (match Stream.peek __strm with
          | Some (Number s) ->
              (Stream.junk __strm; scan_hour_second h m s __strm)
          | _ -> raise (Stream.Error "")))
    | _ ->
  ...
  and scan_tz (__strm : _ Stream.t) =
    ...
    | Some Plus ->
        (Stream.junk __strm;
         (match Stream.peek __strm with
          | Some (Number tz) -> (Stream.junk __strm; tz)
          | _ -> raise (Stream.Error "")))
    | Some Minus ->
        (Stream.junk __strm;
         (match Stream.peek __strm with
  ...
  and scan_date_s n m (__strm : _ Stream.t) =
    match Stream.peek __strm with
    | Some Slash ->
        (Stream.junk __strm;
         (match Stream.peek __strm with
              ...
              (Stream.junk __strm;
               ...
               in
                 (if n >= 1000
                  ...
                  scan_gen rest))
          | _ -> raise (Stream.Error "")))
    | _ -> let rest = __strm in (add_data ~mo: n ~md: n (); scan_gen rest)
  and scan_date_d n (__strm : _ Stream.t) =
    match Stream.peek __strm with
    | Some (Number mo) ->
        (Stream.junk __strm;
         (match Stream.peek __strm with
              ...
              (Stream.junk __strm;
               (match Stream.peek __strm with
                | Some (Number md) ->
                    (Stream.junk __strm;
                     let rest = __strm
                     in (add_data ~y: n ~mo ~md (); scan_gen rest))
                | _ -> raise (Stream.Error "")))
          | _ -> raise (Stream.Error "")))
    | Some (Month mo) ->
        (Stream.junk __strm;
              ...
              (Stream.junk __strm;
               (match Stream.peek __strm with
                | Some (Number y) ->
                    (Stream.junk __strm;
                     let rest = __strm
                     in (add_data ~y ~mo ~md: n (); scan_gen rest))
                | _ -> raise (Stream.Error "")))
          | _ -> raise (Stream.Error "")))
    | _ -> raise Stream.Failure
  and scan_date_m mo md (__strm : _ Stream.t) =
    match Stream.peek __strm with
    | Some Comma ->
        (Stream.junk __strm;
         (match Stream.peek __strm with
  ...
  and scan_opt_meridian (__strm : _ Stream.t) =
    ...
    | _ -> None
  in
    ((try scan_gen tokens with | Stream.Error _ -> invalid_arg "Parse.date");
     let may_get r =
       match !r with | None -> invalid_arg "Parse.date" | Some r -> r in
     let get_default d r = match !r with | None -> d | Some r -> r in
     let month = may_get month
     in
       (if (month < 1) || (month > 12) then invalid_arg "Parse.date" else ();
        {
          year = may_get year;
          month = month;
          day = may_get day;
