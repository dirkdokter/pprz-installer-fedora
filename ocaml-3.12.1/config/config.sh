...
if [ "x$PREFIX" = "x" ]; then PREFIX="/home/dirk/portable-pprz/target"; fi
...
if [ "x$TK_DEFS" = "x" ]; then TK_DEFS=" ${X11_INCLUDES}"; fi
if [ "x$TK_LINK" = "x" ]; then TK_LINK=" -ltk8.5 -ltcl8.5  -ldl ${X11_LINK}"; fi
...
if [ "x$OTHERLIBRARIES" = "x" ]; then OTHERLIBRARIES="unix str num dynlink bigarray systhreads threads graph labltk"; fi
