...
let regexp_case_fold s = Pcre.regexp ~flags: [ `MULTILINE; `CASELESS ] s
  
let quote s = Pcre.quote s
...
let string_match pat s pos =
  try
    let result = Pcre.exec ~rex: pat ~flags: [ `ANCHORED ] ~pos s
    in Some result
  with | Not_found -> None
  
let search_forward pat s pos =
  let result = Pcre.exec ~rex: pat ~pos s
  in ((fst (Pcre.get_substring_ofs result 0)), result)
  
let search_backward pat s pos =
  let rec search p =
    try
      (* `ANCHORED: virtually prepends "^" to the regexp *)
      let result = Pcre.exec ~flags: [ `ANCHORED ] ~rex: pat ~pos: p s
      in ((fst (Pcre.get_substring_ofs result 0)), result)
    with | Not_found -> if p > 0 then search (p - 1) else raise Not_found
  in search pos
...
let matched_string result _ =
   ...
   *)
  (ignore (Pcre.get_substring_ofs result 0); Pcre.get_substring result 0)
...
let matched_group result n _ =
  ...
  (if (n < 0) || (n >= (Pcre.num_of_subs result))
   then raise Not_found
   else ();
   ignore (Pcre.get_substring_ofs result n);
   Pcre.get_substring result n)
  
let group_beginning result n = fst (Pcre.get_substring_ofs result n)
  
let group_end result n = snd (Pcre.get_substring_ofs result n)
...
let tr_templ s = (* Convert \n to $n etc. *)
  ...
  let rec tr l =
    ...
    | Pcre.Delim d :: l' ->
        (assert (d.[0] = '\\');
         let n = (Char.code d.[1]) - (Char.code '0')
         in
           if n = 0
           then "$&" :: (tr l')
    ...
    | [] -> [] in
  let l = Pcre.full_split ~rex: templ_re ~max: (-1) s
  in String.concat "" (tr l)
...
let first_chars s len = String.sub s 0 len
  
let last_chars s len = String.sub s ((String.length s) - len) len
...
;;
*)
let substitute_first pat subst s = (* Do it yourself in the meantime *)
  try
    let substrs = Pcre.exec ~rex: pat s in (* or Not_found *)
    let (match_beg, match_end) = Pcre.get_substring_ofs substrs 0 in
    let replacement_text = subst substrs s
    in
      String.concat ""
        [ string_before s match_beg; replacement_text;
...
let bounded_split expr text num =
  let start =
    try
      ...
      (* or Not_found *)
      let (_, match_end) = Pcre.get_substring_ofs start_substrs 0
      in match_end
    with | Not_found -> 0 in
    ...
    then []
      ...
      then [ string_after text start ]
        ...
        (try
           ...
           (* or Not_found *)
           let (pos, match_end) = Pcre.get_substring_ofs next_substrs 0
           in
        ...
        (try
           ...
           in
             (String.sub text start (pos - start)) ::
               (split match_end (n - 1))
         with | Not_found -> [ string_after text start ])
  in if text = "" then [] else split 0 num
  
let split_delim sep text = bounded_split_delim sep text 0
  
let full_split sep s = Pcre.full_split ~rex: sep ~max: (-1) s
  
let bounded_full_split sep s max =
  let max' = if max <= 0 then (-1) else max
  in Pcre.full_split ~rex: sep ~max: max' s
