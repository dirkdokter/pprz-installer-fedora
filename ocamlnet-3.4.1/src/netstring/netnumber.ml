...
(* string;; *)
(* IEEE representation of fp numbers *)
type fp8 = int64
...
let lt_uint8 x y = if x < y then x >= 0L else (y < x) && (y < 0L)
...
let mk_int4 (c3, c2, c1, c0) =
  ...
  let n0 = Int32.of_int (Char.code c0)
  in
    Int32.logor (Int32.shift_left n3 24)
      (Int32.logor (Int32.shift_left n2 16)
         (Int32.logor (Int32.shift_left n1 8) n0))
...
let mk_int8 (c7, c6, c5, c4, c3, c2, c1, c0) =
  ...
  let n0 = Int64.of_int (Char.code c0)
  in
    Int64.logor (Int64.shift_left n7 56)
      (Int64.logor (Int64.shift_left n6 48)
         (Int64.logor (Int64.shift_left n5 40)
            (Int64.logor (Int64.shift_left n4 32)
               (Int64.logor (Int64.shift_left n3 24)
                  (Int64.logor (Int64.shift_left n2 16)
                     (Int64.logor (Int64.shift_left n1 8) n0))))))
...
let dest_int4 x =
  ...
  let n0 = Int32.to_int (Int32.logand x 0xffl)
  in ((Char.chr n3), (Char.chr n2), (Char.chr n1), (Char.chr n0))
...
let dest_int8 x =
  ...
  let n4 =
    Int64.to_int (Int64.logand (Int64.shift_right_logical x 32) 0xffL) in
  let n3 =
    Int64.to_int (Int64.logand (Int64.shift_right_logical x 24) 0xffL) in
  let n2 =
    Int64.to_int (Int64.logand (Int64.shift_right_logical x 16) 0xffL) in
  let n1 =
    Int64.to_int (Int64.logand (Int64.shift_right_logical x 8) 0xffL) in
  let n0 = Int64.to_int (Int64.logand x 0xffL)
  in
...
let int_of_int4 x =
  (if (x < (-0x4000_0000l)) || (x > 0x3fff_ffffl)
   then cannot_represent name_int_of_int4
   else ();
   Int32.to_int x)
...
let int_of_uint4 x =
  if (x >= 0l) && (x <= 0x3fff_ffffl)
  then Int32.to_int x
  else cannot_represent name_int_of_uint4
  
let name_int_of_int8 = "int_of_int8"
  
let int_of_int8 x =
  if (x >= c_min_int_64) && (x <= c_max_int_64)
  then Int64.to_int x
  else cannot_represent name_int_of_int8
  
let name_int_of_uint8 = "int_of_uint8"
  
let int_of_uint8 x =
  if (x >= Int64.zero) && (x <= c_max_int_64)
  then Int64.to_int x
  else cannot_represent name_int_of_uint8
...
let int4_of_int i = Int32.of_int i
  
let name_uint4_of_int = "uint4_of_int"
  
(* fix sign *)
let uint4_of_int i =
  if i >= 0 then Int32.of_int i else cannot_represent name_uint4_of_int
...
let uint8_of_int i =
  if i >= 0 then Int64.of_int i else cannot_represent name_uint8_of_int
...
let int32_of_uint4 x =
  if x >= 0l then x else cannot_represent name_int32_of_uint4
...
let int32_of_int8 x =
  if (x >= (-0x8000_0000L)) && (x <= 0x7fff_0000L)
  then Int64.to_int32 x
  else cannot_represent name_int32_of_int8
  
let name_int32_of_uint8 = "int32_of_uint8"
  
let int32_of_uint8 x =
  if (x >= 0L) && (x <= 0x7fff_0000L)
  then Int64.to_int32 x
  else cannot_represent name_int32_of_uint8
  
let int64_of_int4 = Int64.of_int32
  
let int64_of_uint4 x =
  if x >= 0l
  then Int64.of_int32 x
  else Int64.add (Int64.of_int32 x) 0x1_0000_0000L
...
let int64_of_uint8 x =
  if x >= 0L then x else cannot_represent name_int64_of_uint8
...
let uint4_of_int32 i =
  (if i < 0l then cannot_represent name_uint4_of_int32 else ();
   int4_of_int32 i)
  
let int8_of_int32 = Int64.of_int32
  
let name_uint8_of_int32 = "uint8_of_int32"
  
let uint8_of_int32 i =
  (if i < 0l then cannot_represent name_uint8_of_int32 else ();
   Int64.of_int32 i)
  
let name_int4_of_int64 = "int4_of_int64"
  
let int4_of_int64 i =
  if (i >= (-0x8000_0000L)) && (i <= 0x7fff_ffffL)
...
let uint4_of_int64 i =
  (if (i < 0L) || (i > 0xffff_ffffL)
   then cannot_represent name_uint4_of_int64
   else ();
   (* sign! *)
   Int64.to_int32 i)
...
let uint8_of_int64 i =
  (if i < 0L then cannot_represent name_uint8_of_int64 else (); i)
...
(* floating point                                                     *)
(**********************************************************************)
let fp8_of_fp4 x = (* Requires O'Caml >= 3.08 *)
  Int64.bits_of_float (Int32.float_of_bits x)
  
let fp4_of_fp8 x = (* Requires O'Caml >= 3.08 *)
  Int32.bits_of_float (Int64.float_of_bits x)
  
let float_of_fp8 x = (* Requires O'Caml >= 3.01 *) Int64.float_of_bits x
  
let float_of_fp4 x = (* Requires O'Caml >= 3.08 *) Int32.float_of_bits x
  
(* Old:
   *  float_of_fp8 (fp8_of_fp4 x)
   *)
let fp8_of_float x = (* Requires O'Caml >= 3.01 *) Int64.bits_of_float x
  
let fp4_of_float x = (* Requires O'Caml >= 3.08 *) Int32.bits_of_float x
...
module BE : ENCDEC =
  struct
    ...
    let read_int4_unsafe s pos =
      ...
      let n0 = Int32.of_int (Char.code (String.unsafe_get s (pos + 3)))
      in Int32.logor x n0
...
*)
    let read_int4 s pos =
      (if (pos < 0) || ((pos + 4) > (String.length s))
    ...
    let read_int8_unsafe s pos =
      ...
      let n0 = Int64.of_int (Char.code (String.unsafe_get s (pos + 7)))
      in Int64.logor x n0
      
    let read_int8 s pos =
      (if (pos < 0) || ((pos + 8) > (String.length s))
       then raise Out_of_range
       else ();
    ...
    let write_int4_unsafe s pos x =
      let n3 = (Int32.to_int (Int32.shift_right_logical x 24)) land 0xff
      in
        (String.unsafe_set s pos (Char.unsafe_chr n3);
         let n2 = (Int32.to_int (Int32.shift_right_logical x 16)) land 0xff
         in
           (String.unsafe_set s (pos + 1) (Char.unsafe_chr n2);
            let n1 = (Int32.to_int (Int32.shift_right_logical x 8)) land 0xff
            in
              (String.unsafe_set s (pos + 2) (Char.unsafe_chr n1);
               let n0 = Int32.to_int (Int32.logand x 0xffl)
               in (String.unsafe_set s (pos + 3) (Char.unsafe_chr n0); ()))))
    ...
    let write_int8_unsafe s pos x =
      let n7 =
        Int64.to_int (Int64.logand (Int64.shift_right_logical x 56) 0xffL)
      in
        (String.unsafe_set s pos (Char.unsafe_chr n7);
         let n6 =
           Int64.to_int (Int64.logand (Int64.shift_right_logical x 48) 0xffL)
         in
           (String.unsafe_set s (pos + 1) (Char.unsafe_chr n6);
            let n5 =
              Int64.to_int
                (Int64.logand (Int64.shift_right_logical x 40) 0xffL)
            in
              (String.unsafe_set s (pos + 2) (Char.unsafe_chr n5);
               let n4 =
                 Int64.to_int
                   (Int64.logand (Int64.shift_right_logical x 32) 0xffL)
               in
                 (String.unsafe_set s (pos + 3) (Char.unsafe_chr n4);
                  let n3 =
                    Int64.to_int
                      (Int64.logand (Int64.shift_right_logical x 24) 0xffL)
                  in
                    (String.unsafe_set s (pos + 4) (Char.unsafe_chr n3);
                     let n2 =
                       Int64.to_int
                         (Int64.logand (Int64.shift_right_logical x 16) 0xffL)
                     in
                       (String.unsafe_set s (pos + 5) (Char.unsafe_chr n2);
                        let n1 =
                          Int64.to_int
                            (Int64.logand (Int64.shift_right_logical x 8)
                               0xffL)
                        in
                          (String.unsafe_set s (pos + 6) (Char.unsafe_chr n1);
                           let n0 = Int64.to_int (Int64.logand x 0xffL)
                           in
                             (String.unsafe_set s (pos + 7)
                                (Char.unsafe_chr n0);
                              ()))))))))
    ...
    (* [u]intn_as_string                                                  *)
    (**********************************************************************)
    let int4_as_string x = let s = String.create 4 in (write_int4 s 0 x; s)
      
    let uint4_as_string x = let s = String.create 4 in (write_uint4 s 0 x; s)
      
    let int8_as_string x = let s = String.create 8 in (write_int8 s 0 x; s)
      
    let uint8_as_string x = let s = String.create 8 in (write_int8 s 0 x; s)
    ...
    let read_fp4 s pos = read_int4 s pos
      
    let read_fp8 s pos = read_int8 s pos
 ...
 *)
    ...
    let read_int4_unsafe s pos =
      ...
      let x = Int32.logor x (Int32.shift_left n1 8) in
      let n0 = Int32.of_int (Char.code (String.unsafe_get s pos))
      in Int32.logor x n0
    ...
    let read_int8_unsafe s pos =
      ...
      let x = Int64.logor x (Int64.shift_left n1 8) in
      let n0 = Int64.of_int (Char.code (String.unsafe_get s pos))
      in Int64.logor x n0
           ...
           (String.unsafe_set s (pos + 2) (Char.unsafe_chr n2);
            ...
            in
              (String.unsafe_set s (pos + 1) (Char.unsafe_chr n1);
               let n0 = Int32.to_int (Int32.logand x 0xffl)
               in (String.unsafe_set s pos (Char.unsafe_chr n0); ()))))
